...
export function AcademicControlCenter() {
  ...
  // Pull-to-refresh handler - refetch dashboard data
  const handlePullRefresh = useCallback(async () => {
    ...
    setWeeklyStats({
      current: weeklyRes.data.totalSeconds,
      goal: weeklyRes.data.goalSeconds,
      percent: weeklyRes.data.goalSeconds > 0
        ? (weeklyRes.data.totalSeconds / weeklyRes.data.goalSeconds) * 100
        : 0,
    });
    ...
    const localHistory = getRecentlyOpened(3);
    if (localHistory.length > 0) {
      setRecentMaterials(localHistory.map(m => ({ ...m, viewedAt: m.viewedAt })));
    } else if (activityRes.data.lastReadMaterial) {
      setRecentMaterials([{ ...activityRes.data.lastReadMaterial, viewedAt: new Date().toISOString() }]);
    }
    setLastReadPage(activityRes.data.lastReadPage || 1);
  }, []);
