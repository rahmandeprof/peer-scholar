...
export function StreakCalendar({ compact = false }: StreakCalendarProps) {
  ...
  useEffect(() => {
    ...
    // Refresh streak data when tab becomes visible (handles midnight crossover)
    ...
    // Periodic refresh every 5 minutes to keep streak data fresh
  ...
  useEffect(() => {
    ...
    const handleClickOutside = (e: MouseEvent | TouchEvent) => {
      ...
      const isInsideCell = Array.from(cellRefs.current.values()).some((cell) =>
        ...
        <div className='flex items-center justify-between mb-4'>
          ...
          <div className='flex items-center gap-4 text-sm'>
            ...
            <div
              ...
              title={`${weeklyActiveDays}/7 days active this week`}
            >
                ...
                <div key={weekIndex} className='flex flex-col gap-1'>
                  {week.map((day, dayIndex) => (
                    <div
                      ...
                      className={`w-4 h-4 md:w-3 md:h-3 rounded-sm ${
                        day.date
