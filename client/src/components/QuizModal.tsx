...
import { X, Check, AlertCircle, Trophy, ArrowRight, Loader2, Sparkles } from 'lucide-react';
      ...
      <div className="bg-white dark:bg-gray-900 rounded-3xl shadow-2xl max-w-2xl w-full max-h-[90vh] overflow-hidden relative flex flex-col animate-pop-in">
        ...
        <div className="flex justify-between items-center p-6 border-b border-gray-100 dark:border-gray-800">
          <div className="flex items-center space-x-2">
            ...
            <span className="font-bold text-lg text-gray-900 dark:text-gray-100">Quick Quiz</span>
          </div>
          <button
            onClick={onClose}
        ...
        <div className="flex-1 overflow-y-auto p-6 md:p-8">
          {loading ? (
            <div className="flex flex-col items-center justify-center py-20">
              ...
              <p className="mt-6 text-gray-600 dark:text-gray-400 font-medium animate-pulse">Generating your quiz...</p>
            </div>
          ) : showResult ? (
            <div className="text-center py-10 relative overflow-hidden">
              ...
              <h2 className="text-4xl font-bold text-gray-900 dark:text-gray-100 mb-4 animate-slide-in" style={{ animationDelay: '0.1s' }}>
                {score === questions.length ? 'Perfect Score!' : score > questions.length / 2 ? 'Great Job!' : 'Quiz Completed!'}
              </h2>
              
              <p className="text-xl text-gray-600 dark:text-gray-400 mb-10 animate-slide-in" style={{ animationDelay: '0.2s' }}>
                You scored <span className="font-bold text-purple-600 dark:text-purple-400 text-2xl">{score}</span> out of <span className="font-bold text-2xl">{questions.length}</span>
              </p>
              
              <div className="flex justify-center space-x-4 animate-slide-in" style={{ animationDelay: '0.3s' }}>
                <button
                  onClick={onClose}
            ...
            <div className={`transition-opacity duration-300 ${animating ? 'opacity-0 translate-y-4' : 'opacity-100 translate-y-0'}`}>
                  ...
                  {questions.map((_, idx) => (
                    <div 
                      ...
                      className={`h-2 w-8 rounded-full transition-all duration-300 ${
                        idx < currentQuestionIndex ? 'bg-purple-600' : 
                        idx === currentQuestionIndex ? 'bg-purple-400 w-12' : 'bg-gray-200 dark:bg-gray-700'
                      }`}
                    />
                  ))}
                </div>
              ...
              <div className="space-y-4 mb-8">
                {questions[currentQuestionIndex].options.map((option, idx) => {
                  ...
                  let buttonClass = "w-full p-5 text-left rounded-2xl border-2 transition-all duration-200 flex justify-between items-center group relative overflow-hidden ";
                  ...
                  if (selectedOption) {
                    if (isSelected) {
                      ...
                      buttonClass += "border-green-500 bg-green-50 dark:bg-green-900/20 text-green-700 dark:text-green-300 shadow-md";
                    } else {
                      buttonClass += "border-gray-100 dark:border-gray-800 opacity-40 grayscale";
                    }
                  } else {
                    buttonClass += "border-gray-200 dark:border-gray-700 hover:border-purple-500 hover:bg-purple-50 dark:hover:bg-purple-900/10 hover:shadow-md transform hover:-translate-y-0.5";
                  }
            ...
            <div className="text-center py-10">
              ...
              <h3 className="text-xl font-bold text-gray-900 dark:text-gray-100 mb-2">Oops!</h3>
              <p className="text-gray-600 dark:text-gray-400 mb-8">Failed to load quiz. Please try again.</p>
              <button
                onClick={onClose}
