...
export default function HelpfulLinksPanel({
  ...
  const fetchLinks = async () => {
    try {
      const res = await api.get(`/helpful-links/material/${materialId}`);
  ...
  const handleMarkHelpful = async (linkId: string) => {
    try {
      await api.post(`/helpful-links/${linkId}/helpful`);
      setLinks((prev) =>
        prev.map((link) =>
          link.id === linkId
            ? { ...link, helpfulCount: link.helpfulCount + 1 }
            : link,
        ),
      );
    } catch (err) {
  ...
  const handleDelete = async (linkId: string) => {
    ...
    try {
      await api.delete(`/helpful-links/${linkId}`);
    ...
    <div className='bg-white/60 dark:bg-gray-800/60 backdrop-blur-xl rounded-2xl p-6 border border-gray-200/50 dark:border-gray-700/50'>
      ...
      {showAddForm && (
        <form
          ...
          className='mb-6 p-4 bg-gray-50 dark:bg-gray-700/30 rounded-xl'
        >
          <div className='space-y-3'>
            <input
              type='url'
      ...
      {links.length === 0 ? (
        ...
        <div className='space-y-4'>
          {links.map((link) => {
            ...
            const embedUrl =
              link.linkType === 'video' ? getYouTubeEmbedUrl(link.url) : null;
