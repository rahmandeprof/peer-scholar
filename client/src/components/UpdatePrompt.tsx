...
export function UpdatePrompt() {
    ...
    return (
        <div className='fixed bottom-20 md:bottom-6 left-1/2 -translate-x-1/2 z-[9999] 
      bg-primary-600 text-white px-4 py-3 rounded-xl shadow-xl 
      flex items-center gap-3 animate-slide-up max-w-sm mx-4'>
            <RefreshCw className='w-5 h-5 flex-shrink-0' />
            <div className='flex-1'>
                <p className='text-sm font-medium'>New version available!</p>
                <p className='text-xs text-primary-100'>Refresh to get the latest features.</p>
            </div>
            <div className='flex gap-2'>
                <button
                    onClick={handleDismiss}
                    className='text-xs px-2 py-1 hover:bg-primary-500 rounded transition-colors'
                >
                    Later
                </button>
                <button
                    onClick={handleUpdate}
                    className='text-xs px-3 py-1 bg-white text-primary-600 font-medium rounded hover:bg-primary-50 transition-colors'
                >
                    Refresh
                </button>
            </div>
