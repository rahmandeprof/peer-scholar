...
const statusConfig: Record<
  ...
  {
    label: string;
    icon: typeof BorderSpinner | typeof CheckCircle;
    className: string;
    isActive: boolean;
  }
> = {
...
export function ProcessingStatusBadge({
  ...
  return (
    <div
      className={`inline-flex items-center gap-1.5 px-2 py-1 rounded-lg text-xs font-medium ${config.className}`}
    >
      <Icon
        className={`w-3.5 h-3.5 ${isAnimated && Icon === BorderSpinner ? '' : isAnimated ? 'animate-spin' : ''}`}
        size='xs'
      />
