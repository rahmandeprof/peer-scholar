...
export class TTSService {
  ...
  private validateVoice(voice: string): string {
    ...
    if (!validVoice) {
      this.logger.warn(
        `Invalid voice "${voice}", using default: ${this.defaultVoice}`,
      );
  ...
  private async getCachedAudio(
    ...
    if (cached) {
      ...
      this.logger.log(
        `Cache HIT for hash=${textHash.substring(0, 8)}..., voice=${voice}`,
      );
    }
  ...
  private async cacheAudio(
    ...
    this.logger.log(
      `Cached audio: hash=${textHash.substring(0, 8)}..., voice=${voice}`,
    );
  ...
  async generateSpeechWithCache(options: TTSOptions): Promise<TTSResult> {
    if (!this.apiKey) {
      throw new BadRequestException(
        'TTS service is not configured. Please set YARNGPT_API_KEY.',
      );
    }
    ...
    this.logger.log(
      `Cache MISS for hash=${textHash.substring(0, 8)}..., voice=${validatedVoice}. Generating...`,
    );
    const audioBuffer = await this.generateSpeechBuffer({
      text,
  ...
  private async generateSpeechBuffer(options: TTSOptions): Promise<Buffer> {
    ...
    if (text.length <= MAX_CHUNK_LENGTH) {
      this.logger.log(
        `Generating TTS: voice=${validatedVoice}, format=${responseFormat}, length=${text.length}`,
      );
    ...
    // Chunking logic
    this.logger.log(
      `Text length (${text.length}) exceeds chunk limit. Chunking...`,
    );
    ...
    for (const [index, chunk] of chunks.entries()) {
      ...
      const buffer = await this.callTtsApi(
        chunk,
        validatedVoice,
        responseFormat,
      );
    ...
    this.logger.log(
      `Successfully combined ${buffers.length} chunks into ${combinedBuffer.length} bytes`,
    );
  ...
  private async callTtsApi(
    ...
    try {
      ...
      this.logger.log(
        `TTS generated successfully for chunk: ${response.data.length} bytes`,
      );
  ...
  async startStreamJob(
    ...
    if (existingJob) {
      ...
      if (existingJob.status === TtsJobStatus.COMPLETED) {
        this.logger.log(
          `Cache HIT: Returning completed job ${existingJob.id} with ${existingJob.totalChunks} chunks`,
        );
        ...
        return {
          ...
          chunkUrls: existingJob.chunkUrls.filter(
      ...
      if (
        existingJob.status === TtsJobStatus.PROCESSING ||
        existingJob.status === TtsJobStatus.PENDING
      ) {
        this.logger.log(
          `Joining in-progress job ${existingJob.id} (${existingJob.completedChunks}/${existingJob.totalChunks})`,
        );
      ...
      this.logger.log(
        `Previous job ${existingJob.id} failed, creating new job`,
      );
    }
    ...
    this.logger.log(
      `Starting new stream job: ${chunks.length} chunks, voice=${validatedVoice}`,
    );
   ...
   */
  async getOrCreateMaterialMeta(
    materialId: string,
    content: string,
  ): Promise<TtsMaterialMeta> {
   ...
   */
  private computeChunkBoundaries(
    content: string,
  ...
  async startMaterialGeneration(
    ...
    const existingChunkMap = new Map(
      existingChunks.map((c) => [c.chunkIndex, c]),
    );
    ...
    this.logger.log(
      `Queued ${chunksToGenerate.length} chunks for material ${materialId}`,
    );
  ...
  async getMaterialChunkStatus(
    ...
    if (!meta) {
      throw new NotFoundException(
        `No TTS meta found for material ${materialId}`,
      );
    }
