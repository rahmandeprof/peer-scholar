...
export class StorageService {
    ...
    constructor(
        ...
        const configuredProvider = this.configService.get<string>('STORAGE_PROVIDER') as StorageProvider;
        ...
        if (this.provider === 'r2' && !this.r2Service.isConfigured()) {
            this.logger.warn('R2 selected but not configured. Falling back to Cloudinary.');
    ...
    async uploadFile(
        ...
        this.logger.debug(`Uploading file via ${this.provider}: ${file.originalname}`);
    ...
    async deleteFile(publicId: string): Promise<void> {
        ...
        const isR2File = publicId.startsWith('scholar-app/') && !publicId.includes('cloudinary');
        const isCloudinaryFile = publicId.includes('cloudinary') || !publicId.startsWith('scholar-app/');
