...
export class StudyGateway implements OnGatewayConnection, OnGatewayDisconnect {
    ...
    private challengeScores = new Map<string, { [userId: string]: { score: number; timeTaken: number } }>();
    ...
    handleChallengeRequest(
        @MessageBody() data: { senderId: string; receiverId: string; materialId: string },
    ) {
    ...
    async handleChallengeResponse(
        @MessageBody() data: { senderId: string; receiverId: string; materialId: string; accept: boolean },
    ) {
        if (data.accept) {
            ...
            try {
                ...
                this.server.to(`user_${data.senderId}`).emit('start_challenge', startPayload);
                this.server.to(`user_${data.receiverId}`).emit('start_challenge', startPayload);
            } catch (e) {
    ...
    handleSubmitScore(
        @MessageBody() data: { challengeId: string; userId: string; score: number; timeTaken: number },
    ) {
