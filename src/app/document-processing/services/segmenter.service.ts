...
export class SegmenterService {
   ...
   */
  segment(text: string, pages?: PageContent[], isOcr = false): TextSegment[] {
    const opts = isOcr ? { ...this.ocrOptions } : { ...this.defaultOptions };
    ...
    this.logger.debug(
      `Created ${segments.length} segments from ${text.length} characters${isOcr ? ' (OCR mode)' : ''}`,
    );
  ...
  private attachPageInfo(
    ...
    for (const segment of segments) {
      ...
      for (const page of pages) {
        ...
        if (
          segmentStart < page.endCharIndex &&
          segmentEnd > page.startCharIndex
        ) {
  ...
  private detectHeadings(segments: TextSegment[]): void {
    for (const segment of segments) {
      ...
      const noPunctuation =
        !/[.!?]$/.test(firstLine) || firstLine.endsWith(':');
      const hasMoreContent =
        lines.length > 1 || segment.text.length > firstLine.length + 50;
