...
export class PdfImageService {
  ...
  async convertToImages(pdfBuffer: Buffer): Promise<Buffer[]> {
    ...
    try {
      ...
      this.logger.log(
        `Converted ${images.length} pages to images using pdftoppm`,
      );

      return images;
    } catch (error) {
      this.logger.error(
        `PDF to image conversion failed: ${error instanceof Error ? error.message : error}`,
      );
      throw error;
    } finally {
  ...
  private runPdftoppm(pdfPath: string, outputPrefix: string): Promise<void> {
    return new Promise((resolve, reject) => {
      ...
      const args = ['-png', '-r', String(this.DPI), pdfPath, outputPrefix];
      ...
      process.on('close', (code) => {
        ...
        if (code === 0) {
          ...
          reject(
            new Error(`pdftoppm failed with exit code ${code}: ${stderr}`),
          );
        }
      });
      ...
      process.on('error', (err) => {
        ...
        reject(
          new Error(
            `Failed to spawn pdftoppm: ${err.message}. Make sure poppler-utils is installed.`,
          ),
        );
      });
    });
