...
export class ProcessingStatusController {
    ...
    async getStatus(@Param('materialId') materialId: string): Promise<ProcessingStatusResponse> {
        ...
        const isReady = material.processingStatus === ProcessingStatus.COMPLETED && segmentCount > 0;
        const canRetry = material.processingStatus === ProcessingStatus.FAILED ||
    ...
    async retryProcessing(@Param('materialId') materialId: string): Promise<{ success: boolean; message: string }> {
        const material = await this.materialRepo.findOne({
            where: { id: materialId },
        });
    ...
    async getSegmentCount(@Param('materialId') materialId: string): Promise<{ count: number }> {
