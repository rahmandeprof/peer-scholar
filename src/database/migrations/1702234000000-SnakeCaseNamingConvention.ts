 ...
 */
export class SnakeCaseNamingConvention1702234000000
  implements MigrationInterface
{
  ...
  public async up(queryRunner: QueryRunner): Promise<void> {
    ...
    await this.safeRenameTable(
      queryRunner,
      'personal_course_materials_material',
      'personal_course_materials',
    );
    await this.safeRenameColumn(
      queryRunner,
      'personal_course_materials',
  ...
  private async safeRenameColumn(
    ...
    try {
      ...
      if (columns.length > 0) {
        ...
        console.log(
          `○ Skipped ${table}.${oldName} (column not found, may already be renamed)`,
        );
      }
    } catch (error: unknown) {
  ...
  private async safeRenameTable(
    ...
    try {
      ...
      if (tables.length > 0) {
        ...
        console.log(
          `○ Skipped table ${oldName} (not found, may already be renamed)`,
        );
      }
    } catch (error: unknown) {
      const message = error instanceof Error ? error.message : String(error);
