...
export class AddResetPasswordFieldsToUser1765039579069 implements MigrationInterface {
    ...
    public async up(queryRunner: QueryRunner): Promise<void> {
        ...
        const hasLowercaseToken = await queryRunner.hasColumn("user", "resetpasswordtoken");
        const hasLowercaseExpires = await queryRunner.hasColumn("user", "resetpasswordexpires");

        if (hasLowercaseToken) {
            await queryRunner.renameColumn("user", "resetpasswordtoken", "resetPasswordToken");
        } else {
            // Check if it already exists (idempotency)
            const hasToken = await queryRunner.hasColumn("user", "resetPasswordToken");
            if (!hasToken) {
        ...
        if (hasLowercaseExpires) {
            await queryRunner.renameColumn("user", "resetpasswordexpires", "resetPasswordExpires");
        } else {
